...
function MainApp({ user, onLogout }: { user: User, onLogout: () => void }) {
  const { customers, debts, payments, expenses, employees, loading, addCustomerAndDebt, payInstallment, addExpense, deleteExpense, archiveDebt, deletePermanentDebt, updateDebt, updatePayment, deletePayment, addEmployee, deleteEmployee } = useAppData();

  useEffect(() => {
    if (loading) return;
    
    // Allow the specific email or any authenticated Google user if you want to test
    // For now, I'll keep your email as admin, but ensure it's matched correctly (case-insensitive)
