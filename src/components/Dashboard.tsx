...
import { Search, Plus, CreditCard, AlertCircle, TrendingUp, Users, DollarSign, Calendar, Clock, CheckCircle2, Phone, MessageCircle, FileText, Filter, Receipt, Trash2, ArrowUpRight, ArrowDownRight, Check, Download, Archive, Pencil } from 'lucide-react';
...
interface Props {
  ...
  onUpdateDebt: (debtId: string, data: {customerName: string, customerPhone: string, itemName: string, costPrice: number, sellPrice: number, downPayment: number, installmentAmount: number, nextDueDate: number, transactionDate?: number, warrantyMonths?: number, serialNumber?: string, vehiclePlate?: string, paymentsList?: Array<{ id?: string; amount: number; paymentDate: number; isDeleted?: boolean }>}) => Promise<void>;
...
export function Dashboard({ customers, debts, payments, expenses, employees, userRole, onAddDebt, onPay, onAddExpense, onDeleteExpense, onArchiveDebt, onDeletePermanentDebt, onUpdateDebt, onUpdatePayment, onDeletePayment, onAddEmployee, onDeleteEmployee }: Props) {
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            <StatCard 
              title="وارد اليوم (الكاشير)" 
              value={todayCashDrawer} 
              subtitle={`المستلم اليوم: ${todayReceived.toLocaleString('en-US')} | مصاريف: ${todayExpenses.toLocaleString('en-US')}`}
              ...
              <table className="w-full text-sm text-right border-collapse">
                <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                  <tr>
                    ...
                    {userRole === 'admin' && (
                      <th className="px-4 py-3 font-bold text-slate-700 text-center">حذف نهائي</th>
                    )}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  ...
                  {archivedDebts.length === 0 && (
                    <tr>
                      <td colSpan={userRole === 'admin' ? 7 : 6} className="px-5 py-12 text-center text-slate-500 font-medium">
...
function DebtDetailsModal({ debt, customer, payments, onClose, onDeletePayment, onUpdatePayment, onEdit, userRole }: { debt: Debt, customer: Customer, payments: Payment[], onClose: () => void, onDeletePayment: (paymentId: string, debtId: string, amount: number) => Promise<void>, onUpdatePayment?: (paymentId: string, debtId: string, amount: number, paymentDate: number) => Promise<void>, onEdit?: (debt: Debt) => void, userRole: UserRole }) {
          ...
          <div className="space-y-3 pt-2">
            {phoneFormatted && (
              <a 
                ...
                target="_blank"
                className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3.5 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 shadow-sm"
              >
            ...
            <button
              ...
              className="w-full bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 py-3.5 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
              <Pencil className="w-5 h-5" />
              تعديل بيانات المعاملة والبيع
            </button>
          </div>
        </div>
